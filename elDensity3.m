function [ Rho3D, xGrid, yGrid, zGrid ] = elDensity3(varargin)
...
switch nargin
    case 2
        ...
        Nx=2*ceil(varargin{2}/2.0)-1; % make odd so the centre of an atom at the origin is exactly the central point on the grid
    case 1
        mldFileName=varargin{1};
...
Ny=Nx; Nz=Nx; %xMin, xMax, yMin, yMax, zMin, zMax
...
% Grid size
gridLengthX=length(xGrid);
gridLengthY=length(yGrid);
gridLengthZ=length(zGrid);

% size of input arguments; will be the same for all
sz=size(ci);

% Reserve memory
Rho3D=zeros(gridLengthX,gridLengthY,gridLengthZ);

% Normlization factor in front of each prim. Gaussian
normA=(2/pi)^0.75;
normB=2.^(l+m+n);
